...
public class PortfolioPanel extends JPanel {
    ...
    public PortfolioPanel(User user){
        ...
        JLabel totalReturn = new JLabel("Total Returns: " +user.calculateTotalReturn() + "%");
        ...
        for (int i = 0; i<rows; i++){
            data[i][0] = user.getInvestments().get(i).getTicker();
            data[i][1] = user.getInvestments().get(i).getDateOfPurchase();
            data[i][2] = String.valueOf(user.getInvestments().get(i).getAmount());
            data[i][3] = String.format("%.2f", user.getInvestments().get(i).getMarketPrice()
                    * user.getInvestments().get(i).getAmount());
        }
