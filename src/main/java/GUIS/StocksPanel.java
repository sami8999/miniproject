...
public class StocksPanel extends JPanel {
    ...
    public StocksPanel(Stock stock, User user){
        ...
        JTextArea stockInformation =new JTextArea("Cash Available: £"+String.format("%.2f",user.getCashBalance())
                + "\n" + "\n" + "\n"+ "Share Price: £"+String.format("%.2f",stock.getMarketPrice()) +
                "\n" + "\n" + "\n" + "Shares bought: " + stock.getAmount());
        ...
        amount.addKeyListener(new KeyAdapter() {
            public void keyReleased(KeyEvent e){
                if(amount.getText().equals("")){
                    ...
                    try{
                        double actualPrice = Integer.parseInt(amount.getText()) * stock.getMarketPrice();
                        if (actualPrice<=0){
        ...
        buyButton.addActionListener(new ActionListener(){
            ...
            try{
                user.buyInvestment(stock, Integer.parseInt(amount.getText()));
        ...
        sellButton.addActionListener(new ActionListener(){
            public void actionPerformed(ActionEvent evt){
                ...
                try{
                    user.sellInvestment(stock, Integer.parseInt(amount.getText()));
                    stockInformation.setText("Cash Available: £"+String.format("%.2f",user.getCashBalance())
                            + "\n" + "\n" + "\n"+ "Share Price: £"+String.format("%.2f",stock.getMarketPrice()) +
                            "\n" + "\n" + "\n" + "Shares bought: " + stock.getAmount());
                }catch (NumberFormatException error){
